...
## Restore & Run — 2026-06 (restore-and-verify pass, no new features)

Repo `prithataherpur-debug/pc-2.0` (branch `main`) restored into workspace and booted in live preview.

**Environment**
- Backend deps installed from `backend/requirements.txt` (skipped the redundant `litellm` pin/`emergentintegrations` resolver conflict; `emergentintegrations` installed via its own index — not imported by server.py anyway).
- `backend/.env`: `MONGO_URL`, `DB_NAME` (local defaults), and `EMERGENT_LLM_KEY` (added — required for Emergent Object Storage used by invoice/receipt PDF generation).
- Frontend deps installed via `yarn`; Expo bundler on port 3000; `EXPO_PUBLIC_BACKEND_URL` wired to preview URL.
- Users seeded on startup: `admin/Admin@2026`, `emp1..emp7/Emp@2026`.

**Verified (iteration 28 + curl)**
- Login screen renders with Pritha Cabinet branding; admin & emp1 login OK.
- Daily call list + status update, sales (cash/online/mixed split), daybook cash-vs-online totals, due collections, Excel exports (daybook.xlsx, backup.xlsx) all working.
- Invoices & money receipts: were 500 due to missing `EMERGENT_LLM_KEY` (object storage init failed before DB insert); fixed by supplying the key — both now persist with PDFs.
